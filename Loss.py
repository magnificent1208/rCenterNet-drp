...
class CtdetLoss(torch.nn.Module):
    ...
    def forward(self, pred_tensor, target_tensor):
        hp_weight = self.loss_weight['hp_weight']
        ...
        # init
        hp_loss, hm_loss, wh_loss, off_loss = 0, 0, 0, 0,


        #新加的hp，跟reg一样用L1
        if reg_weight > 0:
            hp_loss += self.crit_reg(pred_tensor['hp_offset'], target_tensor['hp_mask'],target_tensor['ind'], target_tensor['hp'])
...
#        print(target_tensor['hm'].size())
        ...
        return hp_weight * hp_loss + hm_weight * hm_loss + wh_weight * wh_loss + reg_weight * off_loss #+ ang_weight * ang_loss
