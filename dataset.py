...
class ctDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        #加一个dir点的预测,流程跟center hm 一样
        #删去ang，用hp确定角度
        #把headpoint用中心点的偏置表示，而不是再预测一个关键点！
        #这里代码是抄的multipose里sample/multi_post L91
        hp = np.zeros((self.max_objs,  2), dtype=np.float32) #headpoint
        hp_mask = np.zeros((self.max_objs), dtype=np.uint8)
        ...
        for k in range(num_objs):  # num_objs图中标记物数目  
            ...
            # bbox, an, hpoint = coco_box_to_bbox(ann['bbox'])
            bbox, hpoint = coco_box_to_bbox(ann['bbox'])
            cls_id = int(self.cat_ids[ann['category_id']])


            # 将box坐标转换到 128*128内的坐标
            bbox[:2] = affine_transform(bbox[:2], trans_output)    # 将box坐标转换到 128*128内的坐标
            bbox[2:] = affine_transform(bbox[2:], trans_output)
            hpoint = affine_transform(hpoint, trans_output)
            ...
            # 反复检查这个中心点有没有算对！！ 粗心
            hp_xoff = hpoint[0] - (bbox[0]+0.5*w)
            hp_yoff = hpoint[1] - (bbox[1]+0.5*h)
            ...
            if h > 0 and w > 0:
                ...
                reg[k] = ct - ct_int
                reg_mask[k] = 1
                #hp&hp_mask 基本上步骤跟wh_reg一致即可
                hp[k] = hp_xoff, hp_yoff
                hp_mask[k] = 1 # 搞清楚这个mask怎么弄

        # ret = {'input': inp, 'hp': hp, 'hm': hm, 'reg_mask': reg_mask, 'ind': ind, 'wh': wh, 'ang':ang}
        ret = {'input': inp, 'hp': hp, 'hp_mask': hp_mask, 'hm': hm,
...
def coco_box_to_bbox(box):
    ...
    # ang = float(box[4])
    hpoint = np.array(box[-2:], dtype = np.float32)
    # hpoint = np.array([box[-2],box[-1]],dtype=np.float32)
    # return bbox, ang, hpoint
    return bbox, hpoint
